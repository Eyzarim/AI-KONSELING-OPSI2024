...
export const imageInterpretation = async (token: string = '', imageFile: File) => {
    let error = null;

    // Buat FormData untuk upload file gambar
    const formData = new FormData();
    formData.append('file', imageFile);

    const res = await fetch(`${IMAGES_API_BASE_URL}/interpretasi`, {
        method: 'POST',
        headers: {
            Accept: 'application/json',
            ...(token && { authorization: `Bearer ${token}` })
        },
        body: formData  // Kirim file gambar melalui FormData
    })
        .then(async (res) => {
            if (!res.ok) throw await res.json();
            return res.json();
