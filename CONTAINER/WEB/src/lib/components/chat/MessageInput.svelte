<script lang="ts">
	...
	const interpretImageWithResNet = async (file: File) => {
		const formData = new FormData();
		formData.append('file', file);

		try {
			const response = await fetch(`${WEBUI_API_BASE_URL}/interpretasi`, {
				method: 'POST',
				body: formData,
			});

			if (!response.ok) {
				throw new Error('Failed to interpret image');
			}

			const result = await response.json();
			return result; // Ensure result.text contains the interpretation
		} catch (error) {
			console.error('Error interpreting image:', error);
			toast.error('Failed to interpret image');
	...
	const inputFilesHandler = async (inputFiles) => {
		for (const file of inputFiles) {
			...
			if (['image/jpeg', 'image/png', 'image/jpg'].includes(file['type'])) {
				...
				const res = await interpretImageWithResNet(file);
				if (res) {
					console.log("Interpretasi ResNet berhasil:", res);
					// Simpan hasil interpretasi ke variabel promptInterpretasi
					let promptInterpretasi = `[Image Interpretation]: ${res.text}\n`;

					// Gabungkan hasil interpretasi dengan prompt manual dari pengguna (yang diinput di textarea)
					prompt = `${promptInterpretasi}${prompt}`; // Hasil interpretasi digabungkan dengan prompt pengguna

					// Tunggu pengguna menambahkan prompt tambahan sebelum submit
					toast.success("Gambar berhasil diinterpretasi. Silakan tambahkan prompt dan kirim.");
				}
			} else {
