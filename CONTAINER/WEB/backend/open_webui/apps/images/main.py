...
async def interpret_image(file: UploadFile = File(...)):
    try:
        # Baca konten file gambar yang diupload
        content = await file.read()
        image_bytes = BytesIO(content)

        # Proses gambar menggunakan ResNetModel
        image_tensor = ResNetModelInference.process_image(image_bytes)  # Preprocessing gambar

        # Lakukan prediksi dengan model ResNet
        prediction = ResNetModelInference.predict(image_tensor)  # Prediksi dengan ResNet
        
        return {"Hasil scan gambar dari pengguna dengan menggunakan fitur resnet yang terintegrasi adalah ": prediction}
